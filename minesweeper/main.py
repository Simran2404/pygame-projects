...
win = pygame.display.set_mode((WIDTH, HEIGHT)) #initialise a pygame window with the height and width we specified
...
MINES = 20
SIZE = WIDTH // ROWS
NUM_FONT = pygame.font.SysFont('comicsans', 10)
...
def get_neighbors(row, col, rows, cols):
    ...
    if row < rows - 1 and col > 0:  # bottom left 
        neighbors.append((row + 1, col - 1))
    if row < rows - 1 and col < cols - 1: # bottom right 
        neighbors.append((row + 1, col + 1))  
...
def draw(win, field, cover_field, current_time):
    ...
    win.blit(time_text, (WIDTH/2 - time_text.get_width()/2, 
                    HEIGHT/2 + 4 * time_text.get_height()))
        ...
        for j, value in enumerate(row): 
            ...
            if value > 0: 
                ...
                win.blit(text, (x + (SIZE/2 - text.get_width()/2), 
                                y + (SIZE/2 - text.get_height()/2))) # center the text
...
def uncover_from_pos(row, col, cover_field, field):  # implementing a depth first search here 
    q = Queue()
    q.put((row, col))
    visited = set()
...
def draw_lost(win, text):
    text = LOST_FONT.render(text, 1, "black", "gray")
    win.blit(text, (WIDTH/2 - text.get_width()/2, 
                    HEIGHT/2 + 4 * text.get_height()))
    pygame.display.update()
...
def main():
    ...
    while run: 
        ...
        if lost: 
            ...
            field = create_mine_field(ROWS, COLS, MINES)  # restting values
            cover_field = [[0 for _ in range(COLS)] for _ in range(ROWS)]
            flag_positions = set()
            flags = MINES
            clicks = 0
