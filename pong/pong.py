...
gadget_pair = 1
ch = int(input("Enter your choice of gadget pair"))
if ch == 1:
    gadget_pair = 1
elif ch == 2: 
    gadget_pair = 2
...
#main game loop
while run: 
    wn.fill(BLACK) #erases the old screen so the new position is drawn on the new screen
    ...
    if ball_x >= WIDTH - radius: #the ball is going off on the right hand side 
        player_1 += 1
        ball_x, ball_y = WIDTH/2 - radius, HEIGHT/2 - radius
        dummy_ball_x, dummy_ball_y = WIDTH/2 - radius, HEIGHT/2 - radius
    ...
    if ball_x <= 0 + radius:  #the ball is going off on the left hand side 
        player_2 += 1
        ball_x, ball_y = WIDTH/2 - radius, HEIGHT/2 - radius
        dummy_ball_x, dummy_ball_y = WIDTH/2 - radius, HEIGHT/2 - radius
        second_right_paddle_y = right_paddle_y
    ...
    #paddle collisions
    #left paddle and second left paddle collisions
    ...
    #right paddle and second right paddle collisions
    ...
    #second pair
        ...
        if left_gadget == 1:
            ...
            second_left_paddle_y = left_paddle_y + 200  #paddle cloning
            left_gadget = 0
            left_gadget_remaining -= 1
        ...
        if right_gadget == 1:
            ...
            second_right_paddle_y = right_paddle_y + 200  #paddle cloning
            right_gadget = 0
            right_gadget_remaining -= 1
